...
async function testBlockchainAPI() {
  ...
  try {
    ...
    console.log(`✅ API is reachable: ${response.data.name}`);
...
async function testEmployeeSubmission() {
  ...
  try {
    ...
    if (response.data.success) {
      console.log(`✅ Employee test successful`);
      ...
      console.log(`❌ Employee test failed: ${JSON.stringify(response.data)}`);
      return null;
    }
  } catch (error) {
    console.log(`❌ Employee test error: ${error.message}`);
...
async function testAttendanceSubmission() {
  ...
  try {
    ...
    if (response.data.success) {
      console.log(`✅ Attendance test successful`);
      ...
      console.log(`❌ Attendance test failed: ${JSON.stringify(response.data)}`);
      return null;
    }
  } catch (error) {
...
async function testDataRetrieval(recordId, endpoint) {
  ...
  try {
    ...
    if (response.data.success) {
      ...
      console.log(`❌ Data retrieval failed: ${JSON.stringify(response.data)}`);
      return false;
    }
  } catch (error) {
...
async function runTests() {
  ...
  if (!PRIVATE_KEY) {
    console.log('❌ PRIVATE_KEY is required in .env file');
    return;
  }
  ...
  console.log('================');
  if (allTestsPassed) {
    console.log('✅ All tests passed! Consumer should work correctly.');
  } else {
    console.log('❌ Some tests failed. Check the blockchain API and configuration.');
  }
}
